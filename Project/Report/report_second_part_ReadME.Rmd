...
Table 1 **(in your file it is not directing me to the table)** displays the methods used with the corresponding results on the **leader board**, and, when available, on cross validation. Rather than describing all the techniques we tried, we will concentrate on the ones which allowed us to achieve a particular milestone, and others that seemed relevant for our competition.  

#2.1 One method approach
We first tried multinomial GLMs penalized with Ridge, Lasso and Elastic Net. The performed rather poorly, achieving around 50% on the leaderboard. But we used them as benchmarks for future methods. 
